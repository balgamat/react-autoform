...
describe('Validation', () => {
  it('Outputs object schema according to the fields sent', () => {
    expect(JSON.stringify(createValidationSchema((fields as unknown) as Field<any>[]))).toEqual(
      JSON.stringify(
        validation.object().shape({
          ...
          volume: validation.object().shape({
            ...
            extra: validation.object().shape({ multiplier: validation.number().min(1).max(10) }),
          }),
        }),
      ),
    );
